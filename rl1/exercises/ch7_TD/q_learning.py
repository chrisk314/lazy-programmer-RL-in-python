...
def main() -> int:
    ...
    while True:
        ...
        while s not in env.terminal_states:
            # Choose action according to epsilon-greedy ("behaviour policy")
            a = choose_action_from_Q_epsilon_greedy(Q, s)
            sa_cnt[(s, a)] += 1
            # Get reward and next state resulting from action
            s2, r = env.act(s, a)
            # Update Q value for s and a according to argmax a2 for s2 ("target policy")
            Q_prev = Q[(s, a)]
            Q_max_s2a = max([Q[(s2, _a)] for _a in ACTION_SPACE])
            Q[(s, a)] = Q_prev + ALPHA * (r + GAMMA * Q_max_s2a - Q_prev)
